    ...
    public class FiniteStateMachine : ILexicalAnalyzer
    {
        ...
        Lexem ReservedLexem(ref int row, string substring)
        {
            ...
            else
            {
                ...
                if (code != -1) Lexems.Add(new Lexem(row, substring, code));
        ...
        Lexem Identifier(ref int row, string substring)
        {
            ...
            else
            {
                ...
                else throw new Exception($"Opps,I see a problem.Maybe you forgot about type 'int', 'float'or 'program' type in row {row} near '{substring}'");
            }
